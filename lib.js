...
//랜덤한 정수 반환받기
//호출자는 본인이 반환받기를 원하는 수에 +1을 더해서 호출해아함
...
function getResult(n1, n2) {
    ...
    //이 함수를 어떤한 UI가 호출할지 예측할수가 없으므로,
    //js와 관련된 모든 페이지에서 이 함수를 재사용 하기 위해서는 
    //함수는 중립적일수록 좋다.
    //따라서 반환값을 적극 활용하면 좋다.
    //즉 함수를 호출한 자가 무엇을 할지 예측할 수 없기 때문에
    //반환된 데이터를 가지고 무엇을 할지에 대한 결정은 호출자에게 
    //맡기자
    return result;
}
